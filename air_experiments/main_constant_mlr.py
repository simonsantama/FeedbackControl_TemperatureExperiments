...
that samples pyrolyse at a constant mlr
...
2. Once the mlr is within the desired value, the control of the lamps is 
...
while True:
	try:
		...
		name_of_file = f"air_{number_of_test}_{material}_{mlr_desired}gm-2s-1.csv"
	...
	while time.time() - time_start_logging < time_pretesting_period:
		...
		if time.time() - previous_log < time_logging_period:
			...
			writer.writerows([[t_array[time_step],
				...
				IHF[time_step],
				mlr[time_step],
				mlr_moving_average_array[time_step], 
				message,
				PID_state,
		...
		try:
			...
			if time.time() - previous_log < time_logging_period:
				...
				if PID_state == "not_active":
					...
					if (mlr_moving_average > 0.95*mlr_desired) and (
